...
- This tool is merely a management tool and has no internet connectivity. It does not translate text itself; instead, it uses a third-party API. It does not generate new text, nor does it provide any text itself. All text is input by the user, and all translations are generated by the third-party API.
- The user is responsible for the input and output content. This project should not and cannot review user content, nor is it responsible for user input and output.
- Users bear full responsibility for the content generated using this tool.
...
- 本工具只是一个管理工具，没有联网功能；它本身不会翻译文本，而是通过第三方 API，也不产生新文本，其本身也不提供任何文本，所有文本皆有用户输入，所有翻译皆由第三方 API 生成
- 其输入输出内容由用户负责，本项目不应该也无法审核用户内容，亦不对用户输入输出内容负责
- 用户对使用本工具产生的内容负所有责任
- 本项目是非官方工具，与 KISS / COM3D2 或任何翻译组均无隶属关系。
- 本项目用于管理翻译数据与工作流，不是游戏运行时注入文本的插件。
